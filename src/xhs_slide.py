...
def scrape(req_method, req_uri, req_header, req_params, req_dat) -> requests.Response:
    ...
    try:
        ...
        if resp.status_code == 200:
            return resp
        logger.error(
            f"An error occurred while crawling, status code: {resp.status_code}, req_uri: {req_uri}"
        )
...
class ShuMei:
    ...
    def captcha_js(self):
        ...
        res = scrape(
            req_method="get", req_uri=url, req_header={}, req_dat={}, req_params={}
        )
        js_ver = re.search("-(\d+)/captcha-sdk", res.text).group(1)
        js_url = "https://castatic.fengkongcloud.com" + re.search(
            '"js":"(.*?)"}', res.text
        ).group(1)
